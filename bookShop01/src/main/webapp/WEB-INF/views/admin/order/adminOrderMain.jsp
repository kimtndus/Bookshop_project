...
<%-- 			     <input  type="button" value="배송수정"  onClick="fn_modify_order_state('${item.order_id}','s_delivery_state${i.index}')"/> --%>
			     <button type="button" class="btn btn-outline-secondary" style="width: 70px; margin-left: 3px;" onClick="fn_modify_order_state('${item.order_id}','s_delivery_state${i.index}')">배송수정</button>
			    </td>
			</tr>
		</c:when>
		</c:choose>	
