			...
			<td>
			  <c:choose>
			   <c:when test="${item.delivery_state=='delivery_prepared'}">
			       ...
			       <button type="button" class="btn btn-outline-danger" onClick="fn_cancel_order('${item.order_id}')" value="주문취소">주문취소</button>
			       
			   </c:when>
			   <c:otherwise>
			      <%-- <input  type="button" onClick="fn_cancel_order('${item.order_id}')" value="주문취소" disabled /> --%>
   			       <button type="button" class="btn btn-outline-danger" onClick="fn_cancel_order('${item.order_id}')" value="주문취소" disabled>주문취소</button>
			   </c:otherwise>
			  </c:choose>
			</td>
			</tr>
